...
with col_left:
    ...
    if module == "Eğitim & Akademi":
        ...
        if b1.button("✨ Sunum Hazırla", use_container_width=True):
            ...
            st.session_state['ai_msg'] = "Bu buton Mars Survival unite sunum taslagini kesik cizgili onizlemede acar ve pptx indirmenizi saglar."
            go_rerun(True)
        ...
        if b3.button("🎓 Kelime Bankası", use_container_width=True):
            ...
            st.session_state['ai_msg'] = "Bu buton unite konusunu ozetler, hap bilgiler verir ve PDF indirme sunar."
            go_rerun(True)
        # Kolon render farklari olursa sunum akisini her durumda gorunur kilan yedek buton
        ...
        if st.session_state.get('show_slider'):
            ...
            if st.button("Kutuda Sınavı Oluştur", use_container_width=True):
                ...
                st.session_state['ai_msg'] = f"Bu buton {cnt} soruluk Mars Survival / Conditionals sinavini onizler ve PDF indirir."
                go_rerun(True)
        ...
        if f1.button("🔍 İSG & 5S Analizi", use_container_width=True):
            ...
            st.session_state['ai_msg'] = (
                "Bu buton sahada 5S ve temel ISG uyumsuzluklarini ornek senaryolarla listeler; "
                "PDF rapor indirmenizi saglar."
            )
            go_rerun(True)
        if f2.button("📝 SOP Üretici", use_container_width=True):
            st.session_state['view_state'] = 'sop'
            st.session_state['ai_msg'] = (
                "Bu buton saha operasyonlari icin SOP taslagini adim adim gosterir ve PDF olarak indirmenize izin verir."
            )
            go_rerun(True)
        ...
        if o1.button("🧾 Fatura OCR Analizi", use_container_width=True):
            ...
            st.session_state["ai_msg"] = (
                "Bu buton yuklediginiz fatura veya belgeyi satir satir OCR tablosuna donusturur; "
                "PDF indirme ve (gorsel ise) taranmis PDF secenegi sunar."
            )
            go_rerun(True)
        if o2.button("📊 Excel Raporu Uret", use_container_width=True):
            ...
            st.session_state["ai_msg"] = (
                "Bu buton ofis kalemlerini Excel tablosuna doker; .xlsx indirir ve kisa PDF ozet sunar."
            )
            go_rerun(True)
        if o3.button("📽 Gorsel Sunum Uret", use_container_width=True):
            ...
            st.session_state["ai_msg"] = (
                "Bu buton yuklenen belge adina gore ofis sunum taslagini uretir; pptx ve istege bagli gorselli PDF indirir."
            )
            go_rerun(True)
        ...
        if y1.button("📊 Stratejik Özet Rapor", use_container_width=True):
            ...
            st.session_state['ai_msg'] = "Yonetici ozeti onizlemede; KPI ve verimlilik metrikleri ozetlenir."
            go_rerun(True)
        if y2.button("🚨 Risk Analiz Raporu", use_container_width=True):
            st.session_state['view_state'] = 'risk_report'
            st.session_state['ai_msg'] = "Risk analizi onizlemede; aksiyon ve sorumluluk maddeleri listelenir."
            go_rerun(True)
